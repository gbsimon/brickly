...
function SignInForm() {
	...
	const handleEmailSignIn = async (e: React.FormEvent) => {
		...
		try {
			...
			setAuthError(
				"Failed to send sign-in link. Please check your email address and try again."
			)
			setLoadingProvider(null)
		}
	}
		...
		<div className={`safe ${styles.page}`}>
			<div className={`cardSolid ${styles.card}`}>
				...
				<p className={`subhead ${styles.description}`}>
					Track your LEGO sets and build your collection piece by
					piece
				</p>
				...
				{isCheckingConfig ? (
					...
					<div className={`${styles.status} ${styles.statusError}`}>
						...
						<p className={styles.statusHint}>
							See .env.example for required environment
							variables.
						</p>
					</div>
				) : null}
