...
export default function SetCard({ set, onRemove, onOngoingToggle }: SetCardProps) {
	...
	const handleClick = (e: React.MouseEvent) => {
		...
		if ((e.target as HTMLElement).closest(".remove-button") || (e.target as HTMLElement).closest(".ongoing-button")) {
			return
		}
	...
	return (
		<div className="group relative overflow-hidden cardSolid hover:opacity-90 transition-opacity">
			<button onClick={handleClick} className="w-full text-left focus:outline-none focus:ring-2 focus:ring-blue-500">
				{set.imageUrl && (
							...
							<div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-60 text-white p-2">
								<div className="flex items-center gap-2 text-xs">
									...
									<div className="flex-1 h-2 bg-gray-700 rounded-full overflow-hidden">
										<div className="h-full bg-blue-500 transition-all" style={{ width: `${progressSummary.completionPercentage}%` }} />
									</div>
					...
					{!set.imageUrl && progressSummary && progressSummary.totalParts > 0 && (
						<div className="mt-3 pt-3 border-t border-gray-200">
							<div className="flex items-center justify-between text-sm">
								...
								<span className="font-medium text-gray-900">{progressSummary.completionPercentage}%</span>
							</div>
			...
			<button onClick={handleOngoingToggle} className={`ongoing-button absolute top-2 left-2 w-10 h-10 rounded-full bg-white shadow-md opacity-80 hover:opacity-100 active:opacity-100 transition-opacity flex items-center justify-center ${isOngoing ? "text-blue-600 active:bg-blue-50" : "text-gray-400 active:bg-gray-50"}`} aria-label={isOngoing ? "Mark as not ongoing" : "Mark as ongoing"} type="button">
				{isOngoing ? (
			...
			<button onClick={handleRemoveClick} className="remove-button absolute top-2 right-2 w-10 h-10 rounded-full bg-white shadow-md opacity-80 hover:opacity-100 active:opacity-100 transition-opacity active:bg-red-50 text-gray-600 active:text-red-600 flex items-center justify-center" aria-label="Remove set" type="button">
				<svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			...
			{showConfirm && (
				<div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-10 rounded-lg">
					<div className="bg-white rounded-lg p-6 m-4 max-w-sm w-full shadow-xl">
						...
						<p className="text-sm text-gray-600 mb-4">{t("removeConfirm", { name: set.name })}</p>
						<div className="flex gap-3">
							<button onClick={handleCancelRemove} className="flex-1 rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50" type="button">
								{tCommon("cancel")}
							</button>
							<button onClick={handleConfirmRemove} className="flex-1 rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700" type="button">
								{tCommon("delete")}
							</button>
						</div>
					</div>
